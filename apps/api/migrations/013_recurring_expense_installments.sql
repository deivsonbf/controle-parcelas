...
SELECT
  e.id AS expense_id,
  CASE WHEN e.recurring THEN 1 ELSE gs.installment_number END AS installment_number,
  CASE WHEN e.recurring THEN 1 ELSE e.installments END AS total_installments,
  CASE
    WHEN e.recurring THEN e.total_amount
...
CROSS JOIN LATERAL (
  SELECT
    ...
    )::DATE AS first_due_month
) schedule
CROSS JOIN LATERAL generate_series(
  1,
  CASE WHEN e.recurring THEN 120 ELSE e.installments END
